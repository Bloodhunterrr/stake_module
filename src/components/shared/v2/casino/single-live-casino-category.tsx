...
const LiveCasinoGameListRenderer = ({
                              categoriesIDs,
                              providersIDs,
                              categoryId,
                              providerId,
    ...
    return (
        showNoData && gameList.length === 0 ? <NoDataAvailable info={`Nothing is found`}/>
            : <>
                <div
                    className={`${gameDynamicClass ?? 'items-grid--cols7'} grid__columns--3 grid__column-gap--16`}>
                    {gameList.map((g, i) => (
                        <Fragment key={`${g?.id ?? 'sk'}-${i}`}>
                            <LiveCasinoGameSlot game={g} isLoading={!g}/>
                        </Fragment>
                    ))}
                </div>
...
export default LiveCasinoGameListRenderer;

...
interface GameListRendererProps {
  ...
  categoryId?: number;
  providerId?: number;
  provider_general_codes?: Array<string> | null;
  searchQuery?: string;
  order_by?: "order" | "name";
...
const GameListRenderer = ({
  ...
  providerId,
  searchQuery,
  provider_general_codes,
  order_by = "order",
  onTotalChange,
  skip = !searchQuery &&
    !categoryId &&
    !providerId &&
    provider_general_codes?.length === 0,
  gameDynamicClass,
  showNoData = false,
  ...
  return showNoData && gameList.length === 0 ? (
    <NoDataAvailable info={`Nothing is found`} />
  ) : (
    <>
      <div
        className={`items-grid ${
          gameDynamicClass ?? "items-grid--cols5"
        } grid__columns--3 grid__column-gap--16`}
      >
        {gameList.map((g, i) => (
